...
foreach ($pair in @{version=$version;buildId=(Get-FramekeepBuildId $app);extensionVersion=$manifest.version;source=$root;installDirectory=$app;installedAt=[DateTime]::UtcNow.ToString('o');files=(@((Get-FramekeepFiles).Keys) + @('Framekeep.exe','launcher.json','launcher-build.json','host.cmd','host-manifest.json'))}.GetEnumerator()) { $receipt | Add-Member -NotePropertyName $pair.Key -NotePropertyValue $pair.Value -Force }
